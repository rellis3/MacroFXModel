"""fib_atlas_bot_v2 engine — the PURE, offline-testable pieces: the bot-side
...
has zero I/O; fib_atlas_bot_v2.py owns all of that).
...
shipped, fully priced, in the `fib_atlas_bot_v2_plan` KV artifact. This engine
...
comment block):
  ...
  3. occupied_directions() — hedge-only concurrency (2026-09-03, owner
     ...
     pyramiding blocked outright. `fib_atlas_bot_v2.py`'s own
...
def occupied_directions(open_book: list[dict], ticket_ladder: dict[int, str],
    ...
    this pair, e.g. `{eurgbp, EURGBP, ...}` — see fib_atlas_bot_v2.py's own
