"""fib_atlas_bot_v2 — Asia + Monday range-extension vote portfolio, live/paper on MT5.

FORK of `fib_atlas_bot/fib_atlas_bot.py` (2026-09-18) that swaps the plan
source: instead of polling `fib_atlas_bot_plan`, a KV snapshot server.js
refreshes every 45s (`_refreshFibAtlasPlan`), this bot asks
`fib_local_decision_engine/server.mjs` — a process on THIS SAME MACHINE —
for a decision computed fresh at call time. That closes the plan-staleness
gap the original bot has (confirmed live, 2026-09-18: a 45s-old snapshot can
disagree with what the exact-moment backtest would have said) — same fix
already shipped for Vote Atlas as `volatility_bot_v3`. See
`MD files/LOCAL_DECISION_ENGINE_ARCHITECTURE.md` for the full rationale.

Everything else is UNCHANGED from `fib_atlas_bot.py` — same risk/sizing/
broker/rearm/chandelier logic (`engine.py`, copied verbatim), same MT5
account and same Telegram chat as the live bot (set independently in this
bot's own `fib_atlas_bot_v2_credentials`/`_config` KV keys — nothing is
inherited automatically), separated only by MAGIC (pylego/magics.py) so
positions/orders never cross-contaminate between the two. This bot NEVER
computes a vote, a level, a stop, or a direction from strategy logic — see
`engine.py`'s own docstring for the two things it genuinely does own (rearm
state, chandelier trailing) and why those are left to it. The actual
vote/pricing math (`js/asiaFibAtlasZonePricer.js` / `js/mondayFibAtlasZonePricer.js`'s
`zonesFromLiveAndBook`) is imported VERBATIM by the local engine from the
same file production's own plan producer uses — never a second
implementation to silently drift out of sync.

  python fib_atlas_bot_v2/fib_atlas_bot_v2.py                         # paper mode (default)
  python fib_atlas_bot_v2/fib_atlas_bot_v2.py --live                  # live MT5 (needs creds in fib_atlas_bot_v2_credentials)
  python fib_atlas_bot_v2/fib_atlas_bot_v2.py --local-url http://127.0.0.1:4501   # default shown; override if the local engine runs elsewhere

Universe: `enabled_pairs` if set, else DEFAULT_PAIRS below (the same 16-pair
universe `fib_local_decision_engine/config.json` computes for) — unlike the
original bot, this one DOES carry its own pair-list fallback, because there
is no server-side plan producer defining "whatever pairs the plan carries"
anymore; the local engine only computes for whatever pairs it's asked about.
Config / credentials / status / trade log / decision log all flow through
the dashboard KV exactly like every other bot (fib_atlas_bot_v2_config /
_credentials / _status / _trade_log / _decision_log / _state).
"""
...
from fib_atlas_bot_v2.engine import (                                    # noqa: E402
    RearmTracker, rearm_distance, zone_is_long, chandelier_stop, occupied_directions,
)

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("fib_atlas_bot_v2")

MAGIC = 20260920                                                 # must match pylego/magics.py — pylego/magics_test.py enforces it
DASHBOARD_URL = os.environ.get("DASHBOARD_URL", "http://localhost:3000")
# fib_local_decision_engine/server.mjs, localhost-only -- port 4501 (Vote
# Atlas's own local engine uses 4500; different port so both can run on the
# same machine). Computes both ladders' decisions fresh at call time instead
# of polling fib_atlas_bot_plan's 45-second-old KV snapshot -- see
# MD files/LOCAL_DECISION_ENGINE_ARCHITECTURE.md.
LOCAL_DECISION_URL = os.environ.get("LOCAL_DECISION_URL", "http://127.0.0.1:4501")
REJECT_COOLDOWN_SECS = 60

# fib_local_decision_engine/config.json's own pair list, hand-kept in sync
# (both are small, frozen sets) -- the FALLBACK when enabled_pairs is empty
# on this side too, same convention volatility_bot_v3's own DEFAULT_PAIRS
# uses. Matches server.js's FIB_ATLAS_DEFAULT_PAIRS (the live bot's real
# universe) exactly.
DEFAULT_PAIRS = ["eurusd", "gbpusd", "usdjpy", "audusd", "nzdusd", "usdcad", "usdchf",
...
DEFAULT_CFG = {
    ...
    # 2 -> 1/6 (10min) and 45 -> 3 (2026-09-18, local decision engine fork):
    # plan_secs is now a same-machine HTTP call, not a Railway round-trip, so
    # polling it as often as tick_secs is safe/cheap -- and plan_max_age_hours
    # exists to catch a STALLED source, which should be caught fast when
    # that source is a local process this machine also owns, same tightening
    # volatility_bot_v3 made for the identical reason.
    ...
    # credential, never a literal in source. Set via fib_atlas_bot_v2_config
...
def run(base_url: str, force_live: bool) -> None:
    ...
    ld = LocalDecisionClient(LOCAL_DECISION_URL)
    try:
        cfg = _deep_merge(DEFAULT_CFG, kv.get_json("fib_atlas_bot_v2_config") or {})
    except Exception as e:
    ...
    # No KV plan fetch here at all (2026-09-18, local decision engine fork):
    # the local engine IS the plan source now, polled every plan_secs in the
    # main loop below, same as volatility_bot_v3's identical change.
    # verify_pairs no longer depends on a plan having landed yet either —
    # enabled_pairs (or DEFAULT_PAIRS) is a real list from the first instant.
    plan = None

    if not paper:
        try:
            creds = kv.get_json("fib_atlas_bot_v2_credentials") or {}
        except Exception as e:
            log.error(f"could not reach dashboard to read credentials: {e} — exiting")
            return
        if not creds.get("mt5_account"):
            log.error("live mode but no mt5_account in fib_atlas_bot_v2_credentials — refusing to start. "
        ...
        if not broker.connect(creds.get("mt5_account"), creds.get("mt5_password"),
            ...
            return
        verify_pairs = sorted({str(p).lower() for p in (cfg.get("enabled_pairs") or DEFAULT_PAIRS)})
        if not verify_pairs:
            log.warning("no enabled_pairs configured — skipping startup symbol "
    ...
    plan = plan                            # None until the main loop's first ld.get_plan() call lands
    ...
    try:
        saved_state = kv.get_json("fib_atlas_bot_v2_state") or {}
    except Exception:
        saved_state = {}
    ...
    def _save_state() -> None:
        try:
            kv.put_json("fib_atlas_bot_v2_state", {
                "generatedAt": (plan or {}).get("generatedAt"),
    ...
    try:
        decision_events: list[dict] = list((kv.get_json("fib_atlas_bot_v2_decision_log") or {}).get("events") or [])
    except Exception:
        decision_events = []
    ...
    def _flush_decision_log() -> None:
        ...
        if not decision_dirty["v"]:
            return
        try:
            kv.put_json("fib_atlas_bot_v2_decision_log", {"events": decision_events})
    ...
    while True:
        ...
        # (a) Plan — pull every plan_secs. 2026-09-18: this is now a
        # same-machine call to the local decision engine (computed fresh
        # each time, not a server-cached snapshot), so `generatedAt`
        # legitimately differs on every call by design — gating resync on
        # "did generatedAt change" no longer means anything (it always has),
        # so just resync every successful fetch, same reasoning
        # volatility_bot_v3 used for the identical change. Fails CLOSED, not
        # silently: if the local engine is unreachable, keep trading on the
        # last-known plan rather than guessing — _plan_age_hours below still
        ...
        if nowt - last_status >= cfg.get("status_secs", 30):
            ...
            try:
                ...
                kv.put_status("fib_atlas_bot_v2_status", status)
            except Exception as e:
            ...
            for key in _enabled_keys(cfg, plan):
                ...
                for z in zones:
                    ...
                    tid = broker.enter(pair, direction, z["sl"], z["tp"], lots,
                                       ...
                                       comment=f"FA2[{dedupe_tag}]", dedupe_tag=dedupe_tag)
...
def main():
    global LOCAL_DECISION_URL
    ap = argparse.ArgumentParser(description="MacroFX fib_atlas_bot_v2 (Asia + Monday range-extension vote, local decision engine)")
    ap.add_argument("--live", action="store_true", help="trade live on MT5 (default: paper)")
    ap.add_argument("--url", default=DASHBOARD_URL, help="dashboard base URL (KV/status/config/quotes)")
    ap.add_argument("--local-url", default=LOCAL_DECISION_URL, help="local decision engine base URL")
    args = ap.parse_args()
    LOCAL_DECISION_URL = args.local_url
