...
12. "brief" is the MAIN OUTPUT and READABILITY IS THE #1 GOAL — it must read like a sharp human analyst talking to a non-specialist, NOT a data readout. Use 3-5 SHORT paragraphs SEPARATED BY BLANK LINES (\\n\\n) — NEVER one long block; a 250-word wall of one paragraph is a failure even if every fact is right. Structure it as distinct paragraphs:
   • Para 1 (2-3 sentences): the plain-English "so what" + the trade, in words anyone understands, BEFORE any jargon (e.g. "EUR/USD is stuck just under a heavy options ceiling at 1.1450 and vol is cheap to fade — the lean is to sell pushes toward 1.1450 back to 1.1400, small size, low conviction.").
   • Para 2: the specific levels that matter (the ceiling, the floor, the trade's stop/target).
   • Para 3: the ONE signal that actually carries evidence (vol-risk-premium / momentum / carry) and the single biggest risk that flips the read.
   The FIRST time you use a desk term (gamma, GEX/DEX, max pain, put/call wall, P/C ratio, PIN, a percentile), add a 3-6 word plain gloss right there. The other fields (macroRead, cotRead, etc.) are OPTIONAL supporting detail — fill only the ones you have real data for, leave the rest "".
