...
import { buildIntradayContext as _fpBuildCtx, intradayCone as _fpCone, intradayTally as _fpTally, intradayRealizedZ as _fpRealZ } from './js/forecastPathCore.js';   // forecast-path summary (cone claims API)
...
12. "brief" is the MAIN OUTPUT: 3-5 short, connected paragraphs that read like a sharp human analyst talking, NOT a data readout. START with ONE plain-English sentence that says what's going on and what the plan is, in words a non-specialist understands — the "so what" BEFORE any jargon (e.g. "Nasdaq's stuck in a tight range near 29,500 and the setup leans toward fading pushes to the top — unless the dollar keeps sliding, which flips it to chasing the upside."). Then flow as one story: what's driving THIS pair now → positioning/options context → the specific levels that matter → the trade and the single biggest risk. Reference real numbers, but the FIRST time you use a desk term (gamma, GEX/DEX, max pain, put/call wall, P/C ratio, PIN, a percentile), add a 3-6 word plain gloss right there (e.g. "the call wall at 29,700 — the strike with the most call bets stacked above, where dealers tend to sell into strength"). The other fields (macroRead, cotRead, etc.) are OPTIONAL supporting detail — fill only the ones you have real data for, leave the rest "".
13. HONESTY ABOUT EVIDENCE (non-negotiable). Options walls, max pain, GEX/DEX, gamma-flip, fib confluences, S/R levels and Asia-range breaks are POSITIONING CONTEXT and practitioner heuristics with weak, unproven durable edge — NOT validated signals. Describe where positioning sits and what it MIGHT mean, hedged. NEVER state them as mechanism-of-fact or claim historical reliability that isn't in the snapshot: banned phrasings include "magnetically pulled", "historically precedes a flush/move", "the tape wants to", and asserting "smart money is doing X" as fact (say "the positioning looks like…"). Momentum, carry and the vol-risk-premium are the better-evidenced reads — lean on those when the data supports them.
14. NO FALSE PRECISION. Every stop and target you name must be an ACTUAL level from the snapshot (a wall, pivot, fib, range edge, cluster, avg retail price) — never invent a round-number target like "27,500" that isn't in the data. Round levels the way a person would say them. If you have no real level for a target, say what to watch for instead of fabricating one. Don't quote spurious decimals (e.g. a "114.3× wall strength") as if the precision were meaningful.
...
app.get('/api/yield-coupling', async (req, res) => {
  ...
  try {
    ...
    for (const s of spec) {
      ...
      // Regime-persistence & forward-forecast (the "predict WHEN price follows" test).
      ...
      // Projection gate (the trader's actual method): does today's price FOLLOW
      // yesterday's yield path, and does early tracking predict late tracking?
      // Intraday only (needs time-of-day within the day). Gate hour tunable.
      const gateHour = Math.min(Math.max(parseInt(req.query.gateHour) || 12, 1), 22);
      const projectionGate = isDaily ? null : computeProjectionGate(priceCol, spreadRaw, times, { gateHourUTC: gateHour });
...
// ── Vol-Forecast Benchmark (σ-estimator OOS scorecard) ───────────────────────
// Scores each σ estimator (EWMA / HV / Yang-Zhang / GARCH / HAR-RV) by how well it
// predicts each instrument's realised range, on one IS/OOS split (QLIKE + MSE).
// Pure scoring lives in volForecastBench.js; this route only fetches D1 and loops.
// Same async-job pattern as the other backtests.
const vfbJobs = new Map();
function _purgeStaleVfbJobs() {
  const cutoff = Date.now() - 60 * 60_000;
...
app.post('/api/vol-forecast-bench/run', express.json({ limit: '256kb' }), (req, res) => {
  ...
  const proxy   = ['gk', 'sq', 'parkinson'].includes(b.proxy) ? b.proxy : 'gk';
  const instFilter = b.pair
    ? BT_INSTRUMENTS.filter(i => i.name === String(b.pair).toUpperCase())
    : BT_INSTRUMENTS;

  const jobId     = `vfb_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const startedAt = Date.now();
  _purgeStaleVfbJobs();
  vfbJobs.set(jobId, { status: 'running', startedAt });
  ...
  (async () => {
    try {
      ...
      vfbJobs.set(jobId, { status: 'error', error: msg, startedAt });
    }
  })();
...
// ── Honest-Policy Portfolio — COG's cell-selection on HONEST 1-min fills ──────
// Reproduces both COG-book ingredients (per-cell fade/follow/skip selection +
// multi-pair portfolio) but with honest 1-min TP/SL touch order — no favourable
// mark-to-close. If the netted equity curve is flat/negative, the selection needs
// the optimistic marking to exist; if it rises, it's a real conditional edge.
...
app.post('/api/honest-policy/run', express.json({ limit: '16kb' }), (req, res) => {
  ...
  const startedAt = Date.now();
  for (const [id, j] of honestJobs) if (j.startedAt < Date.now() - 60 * 60_000) honestJobs.delete(id);
  honestJobs.set(jobId, { status: 'running', startedAt });
  const opts = {
    isFrac: Number.isFinite(+isFrac) ? +isFrac : 0.5,
  ...
  (async () => {
    ...
    try {
      ...
      const names = Object.keys(perPair);
      if (!names.length) { honestJobs.set(jobId, { status: 'error', error: 'No pairs evaluated', log, startedAt }); return; }
      const portfolio = _netPortfolio(selStreams);
      honestJobs.set(jobId, { status: 'done', startedAt, result: { ok: true, ...opts, perPair, pairs: names, portfolio, log } });
    } catch (e) { honestJobs.set(jobId, { status: 'error', error: e?.message || String(e), log, startedAt }); }
  })();
  res.json({ ok: true, jobId });
});
app.get('/api/honest-policy/status/:jobId', (req, res) => {
  const job = honestJobs.get(req.params.jobId);
  if (!job) return res.status(404).json({ ok: false, error: 'Job not found or expired' });
  if (job.status === 'running') return res.json({ ok: true, status: 'running', elapsed: Math.round((Date.now() - job.startedAt) / 1000) });
  if (job.status === 'done') return res.json({ ok: true, status: 'done', ...job.result });
  return res.status(500).json({ ok: false, status: 'error', error: job.error, log: job.log });
...
app.post('/api/range-line/run', async (req, res) => {
  ...
  (async () => {
    try {
      ...
      // Exit A/B: same learned policy, four exits (fixed / structural trail /
      // chandelier / scale-out), each judged on OOS + cost-stress. Follow-only
      // trailing; fade keeps the fixed barrier.
      const exitAB = runExitAB(touchesByPair, { policy: book.policy, splitDate: book.splitDate, costByPair });
      // Held-position model: ONE trade through the trend (suppress re-entry while
      // open) → the honest trades/day + Sharpe, not the per-touch over-count.
      const heldPosition = runHeldPosition(touchesByPair, { policy: book.policy, splitDate: book.splitDate, costByPair });
