...
const _CF_EXACT = new Set([
  ...
  'eod_review_v1',           // the last paid end-of-day review (+ the day's earlier ones). A model
                             // call each; the reader presses the button once and expects to find it
                             // there tomorrow morning. Must also be in _worker.js PERMANENT_KEYS.
  ...
  'macro_scorecard_history_v1', // one row per day of the Macro Scorecard's composite + factor scores.
                             ...
                             // be re-derived without replaying every FRED series through every
                             // engine. Must also be in _worker.js PERMANENT_KEYS.
...
function isCfKey(key) {
  ...
  // bot_daily_recon_* are the per-day live-vs-backtest reconciliation results
  // (decision/win-loss match, P&L vs expected) computed from trade_hist_* —
  // expensive to recompute (full multi-year M1 per pair via loadM1ForPair),
  // must survive Railway redeploys the same way its source data does.
  if (key.startsWith('bot_daily_recon_')) return true;
