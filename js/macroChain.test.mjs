...
console.log('[evaluateChain — verdicts]');
{
  ...
  const nqUp = evaluateChain({ ...vals, nq: v(4) });
  ok('real up, Nasdaq up -> broken, the borrowed-time sentence', nqUp.find(l => l.id === 'real-nq').verdict === 'broken' && /borrowed time/.test(nqUp.find(l => l.id === 'real-nq').read));
  ok('real up, Nasdaq down -> holding', evaluateChain({ ...vals, nq: v(-4) }).find(l => l.id === 'real-nq').verdict === 'holding');
  const spxUp = evaluateChain({ ...vals, spx: v(3) });
