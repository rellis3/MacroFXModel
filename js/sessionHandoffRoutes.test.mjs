/**
 * Tests for js/sessionHandoffRoutes.js's fast live-poll cache — same design
 * and same reason for testing it end-to-end against real data as
 * js/sessionPathRoutes.test.mjs (the thing being verified is real-world
 * latency, not mocked timing).
 *   node js/sessionHandoffRoutes.test.mjs
 */
import assert from 'node:assert/strict';
import { boundPacked, getFastLive, liveCache, liveWarming } from './sessionHandoffRoutes.js';
import { TRANSITIONS } from './sessionHandoffEngine.js';
...
console.log('sessionHandoffRoutes');
...
await t('getFastLive eventually warms up with the 3 most-recently-closed handoffs', async () => {
  ...
  assert.ok(r && !r.warming, 'cold start never finished within 120s');
  assert.ok(r.date, 'expected a live date once warm');
  assert.ok(Array.isArray(r.rows));
  assert.ok(r.rows.length > 0 && r.rows.length <= TRANSITIONS.length, `expected 1-${TRANSITIONS.length} rows (one per transition), got ${r.rows.length}`);
  const seen = new Set();
  for (const row of r.rows) { assert.ok(TRANSITIONS.includes(row.transition)); assert.ok(!seen.has(row.transition), 'duplicate transition in live rows'); seen.add(row.transition); }
});
