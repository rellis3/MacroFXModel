/**
 * OU Core — Ornstein-Uhlenbeck Tier-1 primitives (engine #4 of the analytics
 * map, `ANALYTICS_ENGINE_DESIGN.md`). PROMOTED verbatim from `js/mve/ou.js`
 * (2026-07-25): the math was already pure and re-pointable but lived inside
 * the retired MVE corner (null verdict, wired into nothing), which made every
 * new consumer look like it depended on MVE. Now `js/mve/ou.js` re-exports
 * from here — one implementation, zero drift, MVE untouched numerically
 * (`js/mve/mve.test.mjs` still passes byte-identical).
 *
 * What it answers: is this series mean-reverting, how fast (half-life), and
 * what does a current deviation imply as a distribution (probability of
 * reversion, expected gap closed, CIs) — all closed-form.
 *
 * Discrete OU:  Δz_t = a + b·z_{t-1} + ε ,  κ = −b (speed), half-life = ln2/κ.
 *
 * Measurement brick: `ouFit.ok` says "reverting on this sample", not "fade it
 * profitably" — promotion to a signal goes through the harness.
 */
