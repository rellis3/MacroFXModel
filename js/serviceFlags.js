...
export const SERVICES = [
  ...
  { id: 'deskWatch', where: 'server', label: 'Desk watch trigger evaluation',
    cadence: 'every 15 min', cost: 'low', lean: false, on: true,
    feeds: '/api/desk-watch → today.html + evidence.html early-warning panel, and the desk_watch_v1 firing log' },
  ...
  { id: 'motifAlertBacktest', where: 'server', label: 'Motif Telegram alert-funnel backtest regen',
    cadence: 'once on boot, then daily (MOTIF_ALERT_BACKTEST_INTERVAL_SECONDS)', cost: 'high', lean: false, on: true,
    feeds: '/api/analogml/motif-alert-backtest → motif-alert-backtest.html',
    note: 'Shape-matches + M1-path-races every pair\'s full history, same cost class as sessionResearchFull. Off ⇒ the page keeps showing whatever was last generated (R2-backed, survives redeploys) instead of drifting stale with no way to notice.' },
  ...
  { id: 'mveBookForward', where: 'server', label: 'MVE book-layer forward paper tracker (factor-neutral spread sleeves)',
    cadence: 'daily 07:15 London', cost: 'med', lean: false, on: true,
    feeds: '/api/mve-book/forward → mve.html 📒 panel. The only out-of-sample record of the factor-neutral combined 2Y+10Y spread book (MD files/MVE_BOOK_FORWARD_TRACKER.md). Append-only R2 log (mve/book-forward/*).',
    note: 'One tick ≈ 1 min: loads M1 for the 7 USD majors plus the 2Y/10Y sleeves (FRED) and appends yesterday\'s close. Needs FRED_KEY. Switching it off leaves a gap in a forward record that cannot be reconstructed afterwards (days are held through, not back-filled). Paper only — places no orders.' },
  ...
  { id: 'digest', where: 'server', label: 'The 07:00 digest (Telegram, weekdays)',
    cadence: 'checked every 20 min, sent once at 07:00 London', cost: 'low', lean: false, on: true, feeds: 'the five-line morning message and the expected-range calls' },
  { id: 'rangeScore', where: 'server', label: 'Score yesterday\'s expected ranges at the close',
    cadence: 'hourly', cost: 'low', lean: false, on: true, feeds: '/api/range-calls, the digest\'s Yesterday line' },
