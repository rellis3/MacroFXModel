...
// per-component visibility (raw value, rolling z-score, signed/clipped
...
//   2. Rolling z-score each aligned series over LIQUIDITY_SCORE.zWindowDays.
//   3. Apply the input's `sign` so a positive signed z-score always means
//      "more bullish for liquidity", then clip to +/- zClip std devs.
//   4. Weighted average of signed/clipped z-scores (weight-present only —
...
import { applyPublicationLag, forwardFillOnto, rollingZScore, clip } from './nasdaqTransforms.js';
...
export function computeLiquidityScoreSeries(alignedByInputId, tradingDates) {
  const zByInput = {};
  for (const input of LIQUIDITY_INPUTS) {
    zByInput[input.id] = rollingZScore(alignedByInputId[input.id], LIQUIDITY_SCORE.zWindowDays, LIQUIDITY_SCORE.zClip);
  }
