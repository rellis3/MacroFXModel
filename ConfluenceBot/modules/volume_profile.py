...
def compute_npoc_stack(all_m1_bars: list[dict],
                       ...
                       today_high: float,
                       max_days: int = 12,
                       bucket: float = BUCKET_SIZE) -> list[NakedPOC]:
    """
    ...
    for date in sorted_dates:
        ...
        if len(bars) < 10:
            continue
        hist = _build_histogram(bars, bucket)
...
def compute_volume_profile(today_bars: list[dict],
                           ...
                           max_npoc_days: int = 12,
                           bucket: float = BUCKET_SIZE) -> VolumeProfile:
    """
    ...
    today_hist               = _build_histogram(today_bars, bucket)
    ...
    if prev_bars:
        ph = _build_histogram(prev_bars, bucket)
        prev_poc, prev_vah, prev_val = _poc_vah_val(ph)

    # Build nPOC stack
    if all_m1_bars:
        npoc_stack = compute_npoc_stack(all_m1_bars, session_low, session_high,
                                        max_npoc_days, bucket)
