...
def plan_exits(zone, direction: str, price: float,
               ...
               vol_fc: Optional[dict] = None,
               pip: float = 1.0, digits: int = 2) -> tuple[Optional[ExitPlan], str]:
    """
    ...
    pip / digits: instrument pip size and price precision. The SL caps in cfg
      are expressed in PIPS (max_sl_pips / min_sl_pips) and scaled by `pip` to
      price units; all output prices are rounded to `digits`. Gold (pip=1.0,
      digits=2) reproduces the GoldV2 numbers exactly.
    """
    cfg = cfg or {}
    ...
    if sl_dist < min_sl:
        return None, (f'SL too tight ({sl_dist / pip:.1f}p < {min_sl / pip:.1f}p) '
                      f'even from zone edge')
    if sl_dist > max_sl:
        return None, (f'structural SL {sl_dist / pip:.1f}p exceeds cap '
                      f'{max_sl / pip:.0f}p — setup does not fit the risk box '
                      f'(never truncate)')
    ...
    return ExitPlan(
        sl=rnd(sl), tp1=rnd(tp1), tp2=rnd(tp2),
        sl_dist=rnd(sl_dist),
