...
| **Range-line bot (Python)** | `range_line_bot/` | `engine.py` (`RangeSession` — builds the Asia/Monday ladders, one-shot touches, held-position one-per-(src,side) suppression; `session_anchor_epoch` fixed-UTC window matching the freeze) + `range_line_bot.py` (the live loop: three cadences, reuses pylego `KvClient`/`PaperBroker`/`Mt5Broker`/`position_size`/`instruments`/`point_values`, trails the chandelier exit, publishes `range_line_bot_status`). Paper by default, `--live` for MT5. Tested `engine_test.py` (26) + `smoke_test.py` (entry→trail→exit→journal). | consumes `range_line_bot_plan`; imports the rangeline brick + pylego | ✅ built |
