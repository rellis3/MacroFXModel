...
| **OI forward-test tagging** | `js/oiConfluence.js` | `parseOILevels` (pasted `price type` lines → `[{price,type}]`, `normOIType` slugs put/call-wall/max-pain/gamma-flip/hvl), `tagTradeOI` (is an OI level within tol of an entry), `tradePctReturn` (size-independent %), `nearRoundNumber` (the independence flag), `oiAudit` (join accumulated trade log × per-date OI artifact → tagged-vs-untagged expectancy, per type, + the **not-at-round-#** slice that guards against the naked-levels redundancy trap). `oiStoreToLevels` (reuse the index.html OI analyser's computed output — KV `oi_store` per pair: max pain / call+put walls / gamma-flip / HVL — as `[{price,type}]`, so no second manual entry). Pure/offline-tested (`js/oiConfluence.test.mjs`). **Forward test, not backtest** — no historical options-OI exists for spot FX. A 10-min server tick **snapshots `oi_store` into today's dated slot** of `range_line_oi` (no lookahead — captured in the morning; analyser pairs auto-refresh, manual-entry pairs preserved) and rolls the bot's transient `today_closed_trades` into `range_line_trade_log` (canonical `resolveKey` join key). Endpoints `/api/range-line-bot/{oi (manual override), oi/sync, oi-audit}`. UI on `range-zones.html` (auto-sourced levels + "Sync from OI analyser" + running tally). **Second consumer (2026-07):** `oiStoreToLevels` also backs `GET /api/oi-levels` (live, per-instrument, keyed by `resolveKey`), which the **`ConfluenceBot`** reads each state refresh (`_refresh_oi`) to add OI as a **scored confluence source** in `level_matrix.score_zones` (`oi_magnet` credit for wall/max-pain/HVL, smaller `oi_gamma_flip` for the boundary; one credit at the strongest type, round-number-tagged). One JS conversion brick, two readers — the Python bot never re-ports it. Behind the `use_oi` config flag; paper-first (real on indices, unproven on spot FX). | 🔬 forward-testing |
