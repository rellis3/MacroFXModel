...
| **Confluence test** | `js/confluenceTest.js` | `runConfluenceTest` / `confluenceForPair` / `mergeConfluence` — backtest of "does multi-source S/R make price react?". v2 methodology (un-confounded): confluence = **distinct external source kinds** within tol (`levelSources` PDH/PWH/pivots/round/daily-open) — **fib ladder excluded** (its density was the confound); **location-controlled** by fib band (core/mid/outer); reaction measured by **bounce toward mid** (`excMid`) alongside reversion% + after-cost fade edge. Read-only research (does NOT change the live policy). Reuses `runRangeLineAnalyser` (untouched) + `pnlFor` + `collectLevels`. Route `/api/levels-v2/confluence-test`; panel on `telegram-v2.html`. | ✅ built |
