...
| broker / kv / telegram | `pylego/broker/*`, `pylego/kv.py`, … | Remaining Category-B bricks (MT5 connect, `enter`/`stop`, serialize positions, KV client, alert transport) — consolidate the 6+ in-bot copies. **Next: extract the MT5 broker bricks from `bot/regime_bot.py`** (with the Volatility Bot as first consumer). | 🔲 planned (PYTHON_LEGO.md §4–5) |
