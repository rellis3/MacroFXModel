...
| Per-line strategy | `js/perLineStrategy.js` | per-line confidence engine — `extractTouches`, `buildPolicy` (fade/follow/skip per line×condition cell, IS-learned, significance-gated), `tradePnl` (triple-barrier: TP=inner line, SL=outer), `runPerLine` (pooled-IS policy → per-pair OOS book + equity curve + per-pair trade log) | `forecastAnalyserStore` (book orchestrator + routes); imports `metricsCore`, `backtestStats` | ✅ |
