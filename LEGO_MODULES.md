...
| **Range-line analyser** | `js/rangeLineAnalyser.js` | the Forecast-Level per-line strategy applied to RANGE levels, **modules stripped** — `analyseRangeWindow` (emits perLineStrategy-shaped line records off Asia/Monday fib ladders, triple-barrier), `runRangeLineAnalyser`, `runRangeLineBook` (packed M1 → records → pooled-IS policy → per-pair OOS), plus exported `buildRangeLadder` / `LADDER_LEVELS` (shared with the v2 live producer so live & offline build the identical ladder). Reuses `touchFeatures` + `perLineStrategy` + `barUtils` + `fibProjection` + `forecastAnalyser.bucketM1IntoSessions`. Route `/api/range-line/run`; UI `range-line-strategy.html`. Test `js/rangeLineAnalyser.test.mjs`. | ✅ built |
...
| **Levels-v2 live producer** | `levelsV2Engine.js` (root) | `refreshAllPairsV2` / `refreshPairV2` / `loadPolicy` — fetch OANDA M5/M30/D, build the shared ladders, apply the frozen policy via `gradeLevelV2`, write `ai_entries_v2_*`. One producer, one KV namespace. Routes `/api/levels-v2/{learn,refresh,entries,status}`; UI `telegram-v2.html`. Full design: `TELEGRAM_V2.md`. | ✅ built |
