...
| Touch features | `js/touchFeatures.js` | at-the-moment intraday approach features (`createTouchFeatures(cfg)` factory + `TOUCH_FEATURES`: approach efficiency, approach velocity, WaveTrend-at-touch) for fade-vs-continuation; config set on import | `forecastAnalyser`; imports `vumanchuCore` | ✅ |
