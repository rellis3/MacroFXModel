...
| Per-line strategy | `js/perLineStrategy.js` | per-line confidence engine — `extractTouches`, `buildPolicy` (fade/follow/skip per line×condition cell, IS-learned, significance-gated), `tradePnl` (triple-barrier: TP=inner line, SL=outer), `runPerLine` (pooled-IS policy → per-pair OOS book + equity curve + per-pair trade log). **Risk (Sharpe/Sortino/DD/bootstrap) is scored on the DAILY portfolio-PnL series, not per touch** — same-day touches across pairs/lines are simultaneous & correlated, so the independent unit is a trading day (annualise ≈√252); win%/expectancy/PF stay per-trade. (Scoring per-touch annualised by √trade-count overstated Sharpe ~10× and collapsed the bootstrap CI.) | `forecastAnalyserStore` (book orchestrator + routes); imports `metricsCore`, `backtestStats` | ✅ |
| Backtest stats | `js/backtestStats.js` | the standard battery for a trade-PnL series — Sharpe/Sortino/Calmar/CAGR/PF/payoff/win-rate/expectancy/max-DD+duration, **bootstrap CIs** (resample → total/Sharpe CI + P(profit)), **Monte-Carlo** (shuffle order → drawdown distribution); deterministic seeded PRNG | `perLineStrategy`; imports `metricsCore` | ✅ |
...
| **Range-line analyser** | `js/rangeLineAnalyser.js` | the Forecast-Level per-line strategy applied to RANGE levels, **modules stripped** — `analyseRangeWindow` (emits perLineStrategy-shaped line records off Asia/Monday fib ladders, triple-barrier), `runRangeLineAnalyser`, `runRangeLineBook` (packed M1 → records → pooled-IS policy → per-pair OOS). Reuses `touchFeatures` + `perLineStrategy` + `barUtils` + `fibProjection` + `forecastAnalyser.bucketM1IntoSessions`. Route `/api/range-line/run`; UI `range-line-strategy.html`. Test `js/rangeLineAnalyser.test.mjs`. | ✅ built |
