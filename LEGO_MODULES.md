...
| **Range-line confluence producer** | `js/rangeLineConfluenceProducer.js` | `buildConfluenceArtifact` — ships TODAY's structural-confluence level PRICES per instrument (via `rangeLineAnalyser.sessionConfluenceLevels`, the SAME validated code the OOS confluence quality-filter used — no drift) to KV `range_line_confluence`, so the bot's optional confluence entry-gate is checked against the exact levels the backtest validated. Ships level prices (not a source port): the bot does only the trivial proximity count. No-lookahead (prior D1 + prior M1). Pure, injected I/O; tested `js/rangeLineConfluenceProducer.test.mjs`. Consumed by the ≥N confluence gate in `range_line_bot` (default OFF). | ✅ built |
