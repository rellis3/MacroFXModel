...
| **Credit core** | `js/creditCore.js` | the corporate-credit-spread ("credit-Greeks") feature set + risk-appetite gate from an HY OAS series (oldest→newest, pct-points): `creditFeatures` (position percentile · velocity 1d/5d/20d Δbps · acceleration sign · persistence days-in-regime · CCC−BB quality) + `creditGateFromFeatures`/`creditGate` (→ RISK-ON/NEUTRAL/CAUTION/RISK-OFF). Pure; imports `statsCore` (`rollingPercentile`); change/percentile-based so the liquidity-premium level washes out. Design: `docs/CREDIT_SIGNAL_SPEC.md`. Unit-tested `js/creditCore.test.mjs` (28 cases). | `today.html` credit gate (imported as a module → `window.creditBrick`, not copied) ✅; `creditLeadLagEngine` (predictor) ✅; intended: TDE macro gate, `macroCore` (currently its own inline HY-widening rule) 🔲 | 🟡 |
