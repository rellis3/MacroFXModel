...
| **Broker bricks (Python)** | `pylego/broker/paper.py`, `pylego/broker/mt5.py` | one broker surface (`price`/`balance`/`open`/`positions`/`close`): **`PaperBroker`** (in-memory, `check_barriers` runs the triple barrier — fully offline-tested, `paper_test.py`) + **`MT5Broker`** (live connect/order/serialize, extracted from `regime_bot`; native SL/TP exit; py_compile-only — needs a paper-terminal verify). First consumer: `volatility_bot`; the regime bots adopt `MT5Broker` later. | `volatility_bot` | 🟡 paper ✅ / live needs terminal verify |
