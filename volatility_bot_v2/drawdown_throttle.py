...
`applyDrawdownThrottle` (js/levelAtlasVoteReview.js), added 2026-08-30 after
a live-vs-backtest parity audit found this validated lever had never been
implemented live at all.

Validated in analysis/drawdown_throttle_backtest.mjs: tracks this bot's own
realized-balance drawdown from its running PEAK (never resets, unlike
pylego.risk_guard.RiskGuard's daily/monthly reset), and once that drawdown
breaches `trigger_dd`, scales `risk_pct` by `mult` (a gradual de-risk, not a
hard lockout) until the balance recovers to `restore_dd`. Result on the real
portfolio: ~40% shallower drawdown both in-sample and out-of-sample, at a
real, disclosed Sharpe/CAGR cost -- genuine insurance, not a free-lunch
artifact (everything else this session found "too good" turned out to be
partly inflated; this one costs something, which is why it's trusted).
...
drawdown_throttle_test.py).
    ...
    from volatility_bot_v2.drawdown_throttle import DrawdownThrottle
