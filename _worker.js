...
export default {
  async fetch(request, env) {
    ...
    // -- Global Liquidity backtest → proxy to the Railway origin -------------
    ...
    if (path.startsWith('/api/global-liquidity/')) {
      ...
      try {
        ...
        return err('backtest proxy to Railway failed: ' + (e?.message || e), 502);
      }
    }
    ...
    try {
      ...
      if (path === '/api/kv/set' && request.method === 'POST') {
        ...
        try {
          ...
          const STATUS_KEYS = new Set(['regime_bot_status', 'gold_bot_status', 'gold_v2_status', 'regime_bot_v2_status', 'regime_bot_v7_status', 'dyn_anchor_status', 'macro_equity_bot_status', 'volatility_bot_status', 'volatility_ride_status', 'range_line_bot_status']);
      ...
      // -- /api/regime-v7/tg-test --------------------------------
      // Sends a sample entry-alert Telegram message using the current V7 config
      // and the first pair from the live status (or from config pairs list).
      if (path === '/api/regime-v7/tg-test' && request.method === 'POST') {
        if (!env.FX_SCORES) return err('KV not bound', 503);
        try {
          const cfgRaw    = await env.FX_SCORES.get('regime_bot_v7_config');
          const statusRaw = await env.FX_SCORES.get('regime_bot_v7_status');
          const tgCfgRaw  = await env.FX_SCORES.get('tg_config');
          ...
          const token  = (cfg.tg_token  || '').trim() || (shared.token   || '').trim();
          const chatId = (cfg.tg_chat_id || '').trim() || (shared.chatId  || '').trim();
          if (!token || !chatId) return json({ ok: false, reason: 'No Telegram token/chat ID configured — fill in the V7 Telegram fields and save first.' });

          // Pick a pair: first from live status, else first configured pair
          const statusPairs = Object.keys(status.pairs || {});
          const cfgPairs    = cfg.pairs || ['EUR/USD', 'GBP/USD', 'USD/JPY'];
          const pair        = statusPairs[0] || cfgPairs[0] || 'EUR/USD';
          const pairStatus  = (status.pairs || {})[pair] || {};

          // Build a representative entry-alert message (mirrors formatter.py entry_alert)
          const regime     = pairStatus.regime || 'BULL';
          const conf       = pairStatus.conf ?? 78.5;
          const pairDisp   = pair.replace('/', '');
          const isJpy      = pair.includes('JPY');
          const isGold     = pair === 'XAU/USD' || pair === 'NAS100_USD';
          const priceDp    = isGold ? 2 : (isJpy ? 3 : 5);
          const price      = pairStatus.entry ?? 1.08500;
          const pip        = isGold ? 1.0 : (isJpy ? 0.01 : 0.0001);
          const sl         = price - pip * 15;
          const slPips     = Math.abs(price - sl) / pip;
          const paperTag   = status.paper_mode !== false ? ' [PAPER]' : '';
          ...
          const msg = [
            `${emoji} <b>[V7 TEST] ${pairDisp} — ${direction}${paperTag}</b>`,
            `  Regime    : ${regimeLabel} (${conf.toFixed(1)}%)`,
            `  Price     : ${price.toFixed(priceDp)}`,
            `  SL        : ${sl.toFixed(priceDp)}  (${slPips.toFixed(1)}p)`,
            `  Lots      : 0.10`,
...
{"overallBias":"LONG|SHORT|NEUTRAL","conviction":"HIGH|MEDIUM|LOW","convictionScore":5,"headline":"","regime":{"label":"TRENDING|RANGING|BREAKOUT RISK|MEAN-REVERSION|CHOPPY","detail":""},"macroRead":"","yieldCurveRead":"","oiRead":"","garchRead":"","armaRead":"","spreadSignalRead":"","cotRead":"","sessionRead":"","dollarRegimeRead":"","eventRiskRead":"","surpriseRead":"","keyLevels":[{"price":"","type":"CALL WALL|PUT WALL|MAX PAIN|GAMMA FLIP|FIB CONFLUENCE|PIVOT|RANGE HIGH|RANGE LOW","significance":""}],"tradingFramework":"","goodToDoNow":["",""],"avoidNow":["",""],"breakoutTrigger":"","reversionTrigger":"","cleanBreakPotential":"LOW|MEDIUM|HIGH","cleanBreakRationale":"","sentimentPositioning":"","reflexivity":"","riskWarnings":["",""],"tldr":""}`;
      ...
      if (path === '/api/cot-extremes') {
        ...
        const DISAGG = [
          ...
          { name:'COPPER- #1 - COMMODITY EXCHANGE INC.',        alt:['COPPER-GRADE #1 - COMMODITY EXCHANGE INC.','COPPER - COMMODITY EXCHANGE INC.'], sym:'COPPER', label:'COPPER', group:'metals' },
          ...
          { name:'WTI-PHYSICAL - NEW YORK MERCANTILE EXCHANGE',             alt:['CRUDE OIL, LIGHT SWEET - NEW YORK MERCANTILE EXCHANGE','WTI FINANCIAL CRUDE OIL - NEW YORK MERCANTILE EXCHANGE'], sym:'WTI',    label:'CRUDE OIL', group:'energy' },
          { name:'BRENT LAST DAY - NEW YORK MERCANTILE EXCHANGE',           alt:['BRENT CRUDE OIL LAST DAY - ICE FUTURES EUROPE','BRENT LAST DAY FINANCIAL - ICE FUTURES EUROPE','BRENT CRUDE OIL - ICE FUTURES EUROPE'], sym:'BRENT',  label:'BRENT',     group:'energy' },
          { name:'HENRY HUB - NEW YORK MERCANTILE EXCHANGE',                alt:['NAT GAS NYME - NEW YORK MERCANTILE EXCHANGE','NATURAL GAS - NEW YORK MERCANTILE EXCHANGE','HENRY HUB LAST DAY FIN - NEW YORK MERCANTILE EXCHANGE'], sym:'NATGAS', label:'NAT GAS',   group:'energy' },
          { name:'GASOLINE RBOB - NEW YORK MERCANTILE EXCHANGE',            alt:['GASOLINE BLENDSTOCK (RBOB) - NEW YORK MERCANTILE EXCHANGE','NY HARBOR RBOB GASOLINE - NEW YORK MERCANTILE EXCHANGE'], sym:'RBOB',   label:'GASOLINE',  group:'energy' },
        ...
        const TFF = [
          ...
          { name:'RUSSELL E-MINI - CHICAGO MERCANTILE EXCHANGE',       alt:['E-MINI RUSSELL 2000 - CHICAGO MERCANTILE EXCHANGE','RUSSELL 2000 MINI INDEX FUTURES - CHICAGO MERCANTILE EXCHANGE'], sym:'RTY', label:'RUSSELL 2K', group:'equities', flip:false },
          { name:'UST 10Y NOTE - CHICAGO BOARD OF TRADE',              alt:['10-YEAR U.S. TREASURY NOTES - CHICAGO BOARD OF TRADE','ULTRA UST 10Y - CHICAGO BOARD OF TRADE'], sym:'TY', label:'10Y T-NOTE', group:'rates', flip:false },
          { name:'UST BOND - CHICAGO BOARD OF TRADE',                  alt:['30-YEAR TREASURY BONDS - CHICAGO BOARD OF TRADE','U.S. TREASURY BONDS - CHICAGO BOARD OF TRADE'], sym:'US', label:'30Y T-BOND', group:'rates', flip:false },
          { name:'UST 2Y NOTE - CHICAGO BOARD OF TRADE',               alt:['2-YEAR U.S. TREASURY NOTES - CHICAGO BOARD OF TRADE'], sym:'TU', label:'2Y T-NOTE', group:'rates', flip:false },
      ...
      if (path === '/api/trade-history' && request.method === 'GET') {
        ...
        const BOT_KEYS = ['bot_status', 'regime_bot_status', 'gold_bot_status', 'gold_v2_status', 'regime_bot_v2_status', 'regime_bot_v7_status', 'dyn_anchor_status', 'macro_equity_bot_status', 'volatility_bot_status', 'range_line_bot_status'];
        const dates = [];
