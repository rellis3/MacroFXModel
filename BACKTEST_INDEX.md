...
| `credit-stress.html` | Dashboard → 🚦 Credit Stress | `js/creditStressCore.js` + `js/creditStressEngine.js` | CSI risk overlay: equal-weight z of (BBB−AAA, HY OAS, VIX) gates exposure ×1/×0.5/×0 at frozen 1σ/2σ tiers; benchmark = same gate on VIX alone; primary target = equal-weight long-ccy basket | 🧪 New (2026-07-18). **Pre-registered one-shot** (`CREDIT_STRESS_TEST.md`; odds ~30–40%, "VIX was enough" a live outcome). A risk overlay, NOT alpha — directional use not claimed or tested. Run once on Railway, record the 3-way verdict — Q12. |
...
| **Q12** | Does the credit-stress composite (CSI) beat a VIX-only gate as a sizing overlay OOS? | Run `credit-stress.html` once on Railway, fill in `CREDIT_STRESS_TEST.md` §Result | CSI-gated OOS Sharpe > ungated AND > VIX-gated on the risk basket → sizing-input candidate | `vix-enough` → use VIX, retire CSI · `no-gate` → banked null; no tier/weight iteration |
