...
| `credit-stress.html` | Dashboard → 🚦 Credit Stress | `js/creditStressCore.js` + `js/creditStressEngine.js` | CSI risk overlay: equal-weight z of (BBB−AAA, HY OAS, VIX) gates exposure ×1/×0.5/×0 at frozen 1σ/2σ tiers; benchmark = same gate on VIX alone; primary target = equal-weight long-ccy basket | 🧪 **Run 1 (2026-07-18) INVALID — data, not verdict:** FRED serves ICE BofA OAS with only a trailing ~3y window, so the CSI didn't exist before mid-2024 and the gate slept through 2008/2011/2020. Components amended to Moody's `BAA10Y`/`AAA10Y` (daily since 1986) per the dated note in `CREDIT_STRESS_TEST.md`; criteria untouched. The one valid shot is still pending — Q12. |
...
| **Q12** | Does the credit-stress composite (CSI) beat a VIX-only gate as a sizing overlay OOS? (Run 1 invalid — ICE OAS history restricted on FRED; components amended to Moody's Baa/Aaa, criteria untouched) | Re-run `credit-stress.html` once on Railway post-amendment, fill in `CREDIT_STRESS_TEST.md` §Result | CSI-gated OOS Sharpe > ungated AND > VIX-gated on the risk basket → sizing-input candidate | `vix-enough` → use VIX, retire CSI · `no-gate` → banked null; no tier/weight iteration |
