"""volatility_bot_v3 — Level Atlas Vote Portfolio, live/paper on MT5.
...
consumes the frozen `volatility_bot_v3_plan` (built server-side by
...
that module's docstring).
  ...
  python volatility_bot_v3/volatility_bot_v3.py            # paper mode (default)
  python volatility_bot_v3/volatility_bot_v3.py --live      # live MT5 (needs creds in volatility_bot_v3_credentials)
...
every other bot (volatility_bot_v3_config / _credentials / _status).
"""
...
from volatility_bot_v3.engine import VoteSession, stack_conflict  # noqa: E402
from volatility_bot_v3.currency_gate import CurrencyLossGate    # noqa: E402
...
# pointed at Railway for KV/status/config/quotes exactly as v2 always used
# it; this one is the same-machine process that computes zone decisions
# fresh at call time instead of serving a 45s-old remote snapshot. See
# MD files/LOCAL_DECISION_ENGINE_ARCHITECTURE.md.
LOCAL_DECISION_URL = os.environ.get("LOCAL_DECISION_URL", "http://127.0.0.1:4500")
...
# the real gate is now the LOCAL decision engine's own config.json pair
# list (2026-09-18 — v2's server-side plan producer no longer exists for
# this bot), so a mismatch here is cosmetic, not a safety issue: this bot
# only ever trades what's actually IN the plan the local engine returns.
...
DEFAULT_CFG = {
    ...
    "plan_secs": 3,                  # 2026-09-18: was 45 (a remote KV poll) — now a same-machine
                                     # call to the local decision engine, computed fresh each time,
                                     # so this can match tick_secs instead of trailing it by up to
                                     # 45s. See MD files/LOCAL_DECISION_ENGINE_ARCHITECTURE.md — one
                                     # of several confirmed contributors to v2's live-vs-backtest
                                     # margin divergence (measured directly: a real pending zone's
                                     # margin drifted 3->1 over 43 minutes with no touch occurring).
    "status_secs": 30,
    "tick_secs": 3,
    ...
    # `generatedAt` is now stamped fresh by the LOCAL engine on every
    # successful fetch (2026-09-18), so this measures "how long since the
    # last successful local fetch" rather than "how old is the server's
    # snapshot" — tightened from v2's 1h to 10 minutes: a local process that
    # can't answer for that long is genuinely down, not just having a slow
    # tick, and this bot should stop opening new positions well before an
    # hour of that.
    ...
    # throttle_reset_ack in volatility_bot_v3_state) and only resets once
...
def run(base_url: str, force_live: bool) -> None:
    ...
    try:
        cfg = _deep_merge(DEFAULT_CFG, kv.get_json("volatility_bot_v3_config") or {})
    except Exception as e:
    ...
    if not paper:
        try:
            creds = kv.get_json("volatility_bot_v3_credentials") or {}
        except Exception as e:
            log.error(f"could not reach dashboard to read credentials: {e} — exiting")
            return
        if not creds.get("mt5_account"):
            log.error("live mode but no mt5_account in volatility_bot_v3_credentials — refusing to start. "
    ...
    try:
        saved_state = kv.get_json("volatility_bot_v3_state") or {}
    except Exception:
        saved_state = {}
    ...
    def _save_state() -> None:
        try:
            kv.put_json("volatility_bot_v3_state", {
                "generatedAt": (plan or {}).get("generatedAt"),
    ...
    try:
        decision_events: list[dict] = list((kv.get_json("volatility_bot_v3_decision_log") or {}).get("events") or [])
    except Exception:
        decision_events = []
    ...
    def _flush_decision_log() -> None:
        ...
        if not decision_dirty["v"]:
            return
        try:
            kv.put_json("volatility_bot_v3_decision_log", {"events": decision_events})
    ...
    def _sync_sessions(new_plan) -> None:
        ...
        state on restart (double-entry protection) — same intent as oi_bot's
        _sync_sessions, but NOT gated on generatedAt matching (2026-09-18):
        the local decision engine restamps generatedAt fresh on every single
        call by design, so it can never equal a value saved before a
        restart — that gate would silently disable restart protection
        entirely, not make it safer. zone_id itself already encodes
        pair/date/side/rung/instance, so it's specific enough on its own —
        a restored entered zone_id only ever matches a zone actually
        present in the CURRENT plan (set_zones, below); a genuinely stale
        entry from a prior day can't collide since the date is baked in."""
    ...
    while True:
        ...
        # legitimately differs on every call by design — gating
        # _sync_sessions on "did generatedAt change" no longer means
        # anything (it always has), so just resync every successful fetch.
        # Fails CLOSED, not silently: if the local engine is unreachable or
        # reports stale local data, keep trading on the last-known plan
        # (existing behavior, unchanged) rather than guessing.
        if nowt - last_plan >= cfg.get("plan_secs", 3) or plan is None:
            enabled = cfg.get("enabled_pairs") or DEFAULT_PAIRS
            try:
                new_plan = ld.get_plan(enabled)
            except Exception as e:
        ...
        if nowt - last_status >= cfg.get("status_secs", 30):
            try:
                cfg = _deep_merge(DEFAULT_CFG, kv.get_json("volatility_bot_v3_config") or cfg)
                _apply_broker_symbols(cfg)
                _apply_paper_spreads(broker, cfg)
                guard.sync_cfg(cfg)
            ...
            try:
                ...
                kv.put_status("volatility_bot_v3_status", status)
            except Exception as e:
                    ...
                    if instr in _plan_instruments(plan):
                        ...
                        if nowt - first_seen > MISSING_GRACE_SECS and not warned_missing.get(instr):
                            ...
                            log.warning(f"enabled pair {instr!r} still not in the plan after "
                                        ...
                                        f"(GET volatility_bot_v3_plan's 'skipped' field for the reason)")
                    continue
...
def main():
    ...
    ap = argparse.ArgumentParser(description="MacroFX volatility_bot_v3 (Level Atlas Vote Portfolio, local decision engine)")
