...
// T7b (2026-09-22): the bands are the FITTED LADDER -- the same buildLadder call the
// bot plan, the ladder export and the owner's chart use -- not trailing-250 quantiles.
const ASSET_CLASS = { EURUSD: 'fx', GBPUSD: 'fx', USDJPY: 'fx', AUDUSD: 'fx', USDCAD: 'fx', GOLD: 'commodity', NQ: 'index', SPX500: 'index' };
...
for (const [inst, key] of Object.entries(INSTR)) {
  ...
  for (let i = 250; i < day.length; i++) {
    const d = day[i];
    const priorBars = day.slice(Math.max(0, i - 260), i).map(x => ({ open: x.o, high: x.h, low: x.l, close: x.c }));   // strictly before this session
    const est = paramsFor(inst, ASSET_CLASS[inst])?.estimator ?? 'yz_30';
    const sigma = forecastSigma(priorBars, est); if (!(sigma > 0)) continue;
    const lad = buildLadder(sigma, { instrument: inst, assetClass: ASSET_CLASS[inst], horizon: 'daily', eventTag: 'none' });
    const oh = lad?.oh, ol = lad?.ol; if (!oh?.p50 || !ol?.p50) continue;
    const bands = { upMed: d.o * (1 + oh.p50 / 100), up75: d.o * (1 + oh.p75 / 100), up90: d.o * (1 + oh.p90 / 100), dnMed: d.o * (1 - ol.p50 / 100), dn75: d.o * (1 - ol.p75 / 100), dn90: d.o * (1 - ol.p90 / 100) };
  ...
  const out = { n: rec.length, byCheckpoint: {}, clock: {}, asia: {}, asiaCuts: null };
  // 3. clock
  ...
  // the tercile cut-points, so a live session can be classified against the same
  // boundaries the study used (the digest needs this at 07:00; without it the
  // conditioner is unusable outside the study -- T7b/2026-09-22)
  out.asiaCuts = { low: +q1.toFixed(4), high: +q2.toFixed(4), n: withAsia.length };
...
// of being the session extreme); and the Asia-range conditioner. Bands are the FITTED
// LADDER (buildLadder on forecastSigma of the prior sessions, eventTag none) -- the same
// O-H / O-L p50/p75/p90 lines the bots, the ladder export and the chart use. Base rates, never entries.
