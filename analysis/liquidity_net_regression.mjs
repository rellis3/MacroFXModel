...
const FRED_SERIES = { walcl: 'WALCL', tga: 'WTREGEN', rrp: 'RRPONTSYD', vix: 'VIXCLS', hy: 'BAMLH0A0HYM2', nfci: 'NFCI' };
...
function printResultsTable(rows, indent = '') {
  console.log(`${indent}Horizon  n(IS/OOS)   β         IS t(NW)   IS R²    OOS R²    OOS hit-rate   Verdict`);
  for (const r of rows) {
    const row = r.n
      ? `${indent}${String(r.horizonWeeks + 'w').padEnd(8)} ${String(r.nIS + '/' + r.nOOS).padEnd(11)} ${r.beta.toFixed(4).padStart(8)}  ${r.tStatNW.toFixed(2).padStart(9)}  ${r.r2IS.toFixed(3).padStart(7)}  ${(r.r2OOS ?? NaN).toFixed(3).padStart(8)}   ${(r.hitRateOOS * 100).toFixed(1).padStart(10)}%   ${r.verdict}`
      : `${indent}${String(r.horizonWeeks + 'w').padEnd(8)} ${r.verdict}`;
    console.log(row);
  }
}

function printRegimeSection(regime) {
  if (!regime) { console.log('\n(No VIX/HY OAS data available — regime-conditioned split skipped.)'); return; }
  console.log(`\nRegime split (education/macro-deep-dives-notes.md Lesson 3 — "regime conditioning is mandatory"):`);
  console.log(`Stress = ${regime.stressWeeks}w (${(regime.stressShare * 100).toFixed(1)}%) across ${regime.stressEpisodes} independent episode(s) — credit (HY OAS) or vol (VIX) z-score > 2.0. Calm = ${regime.calmWeeks}w.\n`);
  console.log('  STRESS weeks:'); printResultsTable(regime.resultsStress, '  ');
  console.log('  CALM weeks:'); printResultsTable(regime.resultsCalm, '  ');
  console.log(`\n  ⚠️  ${regime.caveat}`);
}
...
function printReport(result) {
  ...
  printRegimeSection(result.regime);

  if (result.nfci) {
    console.log('\n' + '='.repeat(78));
    console.log('NFCI (Chicago Fed Financial Conditions) → forward Nasdaq returns — second proxy');
    console.log('='.repeat(78));
    console.log(`\n${result.nfci.note}\n`);
    printResultsTable(result.nfci.results);
    printRegimeSection(result.nfci.regime);
  } else {
    console.log('\n(No NFCI data available — second-proxy test skipped.)');
  }
