...
**Update — priced through the real backtest pipeline (`BACKTEST.md`):** run with real position sizing (1% risk/trade), the house cost model, and a one-signal-per-day cap (needed to stop one volatile session firing this rule 66 times), this cell is a clear loser on every instrument, gross or net — Sharpe -0.06 to -1.5 gross, -2.2 to -4.7 net. The one-signal-per-day restriction itself gives up most of the pooled gross edge; see `BACKTEST.md` for the full tearsheet and why.
...
10. **A swing high or low that gets fully retested later in the day breaks slightly more often than it holds, on all three instruments, and the gap does not pay the spread.** Across roughly 50,000 retests per instrument, price takes out the level first 52% of the time on gold, 52% on NQ, 51% on EURUSD, versus rejecting first 48%, and this is stable in-sample vs out-of-sample to within a point on every instrument. This is a different pattern from point 5 above: it is a *later* revisit of a swing after price already moved on, not the immediate pullback right after the leg forms, and it looks close to a coin flip rather than the small edge the fib pullback shows. Both directions (bet the level holds, bet it breaks) are within +/-0.02R gross of zero and negative net; the closest to working is betting *large* swings (>0.7 ADR) break, gross +0.05 to +0.26R depending on instrument, but on small samples (n=857-959). A retest arrives a median 29-45 minutes after the swing confirms, and does not get materially more reliable the longer it takes.
11. **Merging six independent-looking signals into one vote adds real accuracy only on gold, and even there it is fragile.** At 13:00 UK, momentum from the open, side of session VWAP, Asia-range break direction, PDH/PDL touch reaction, the last resolved swing retest and the last resolved fib-leg outcome were each cast as one vote, predicting the sign of checkpoint-to-close (overlap-free with every input). No single signal alone clears 53%. When all active signals agree (roughly one day in five), accuracy is 56.3% on gold [51.8-60.6], with a genuinely good-looking sim (0.5 ADR stop, 1 ADR target): net +0.10R, gross +0.13R, t=2.9 - the single best number in this suite. But: it does NOT replicate on NQ (51.0%, n=347) or EURUSD (51.7%, n=520), it does not clear significance out-of-sample even on gold alone (OOS 55.4%, p=0.15, n=195), the relationship with vote strength is not monotonic (a 3-of-6 majority scores *below* 50% on gold, n=395), and pairwise agreement between the six signals runs 52-84% (median 66-71% across instruments) where genuinely independent ~51%-accurate signals would agree roughly 51% of the time by chance. That last number is the real finding: these six signals mostly detect the same underlying "today is trending" state rather than confirming each other independently, so a unanimous vote is closer to "one big trend day, six confirmations of it" than "six separate pieces of evidence." Restricting to heavy-vol days pushes gold's high-conviction accuracy to 54% versus 48.7% in quiet regimes, consistent with that reading. Confluence here is a filter for conviction, not a source of new edge - worth using to sit out low-agreement days, not to size up on high-agreement ones, and worth re-testing with a longer or walk-forward-only OOS window before trusting the gold number at all.
12. **Once price is 0.3 ADR from the open, it never comes back that day 41-42% of the time, on all three instruments.** At 13:00 UK with more than 0.5 ADR of travel, the close is back through the open only 6-15% of the time. The direction is set by then, but the probability of *further* progress from that point is still 50%.
