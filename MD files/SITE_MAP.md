...
| `level-atlas-vote-backtest.html` | Level Atlas vote-margin backtest — full standard-backtest tearsheet (KPI grid, cost diagnostics, session/hour/day-of-week breakdowns, CSV exports) plus real M1 candles with entry/exit markers and target/stop lines | 🔬 Research | 2026-08-26 | Fixed-barrier P&L (not MFE/MAE), walk-forward positive at margin≥3-4 across 5 pairs (Sharpe 2.3-4.7 uncapped, 1.3-2.4 with a real concurrent-position cap) — see `MD files/LEGO_MODULES.md` for the full result and the Vote Portfolio page for the combined-pair view |
| `level-atlas-vote-portfolio.html` | Combines multiple pairs' own concurrency-capped vote-backtest trades into one portfolio (equal-weight or risk-parity via inverse-vol) | 🔬 Research | 2026-08-27 | Real diversification benefit: 5-pair average Sharpe 1.78 → combined 3.1-3.5, max DD under -1.4%, smaller total return per pair unless levered to a target vol — see `LEGO_MODULES.md` |
| `asia-fib-atlas-live.html` | Live candlestick chart + today's Asia-range fib-extension ladder overlaid, each rung confidence-colored against the OOS-held reference book | 📚 Reference | 2026-08-27 | Two dimensions (same-day retest persistence, late-NY/pre-Asia handoff) confirmed OOS-held on all 26 locally-cached pairs and drive the color; other reads are level-specific, shown in the detail panel not the color — descriptive, not a fitted/backtested signal, see `LEGO_MODULES.md` §1aq |
| `asia-fib-atlas-vote-backtest.html` | Barrier-priced OOS trade backtest (real candles, entry/target/stop markers, full tearsheet incl. bootstrap/Monte-Carlo outcome uncertainty, CSV exports) across all 26 pairs, Asia ladder + Monday-ladder sibling | 🔬 Research | 2026-08-28 | margin=2 (both proven dims agree) holds cross-instrument after a realistic concurrency cap; margin=1 (either alone) does not — see `LEGO_MODULES.md` §1aq |
| `asia-fib-atlas-vote-portfolio.html` | Combines multiple pairs' own Fib Atlas vote trades into ONE portfolio (weighting, concurrency/heat cap, drawdown throttle, non-compounded split, Performance Summary, CSV exports) — Level Atlas Vote Portfolio's sibling, Asia/Monday/Both-combined toggle | 🔬 Research | 2026-08-29 | Deflated Sharpe Ratio check found the headline Sharpe/CAGR numbers do NOT survive correction for the ~104-combination search this vote rule was implicitly selected from (DSR=0 for EURUSD Asia margin≥2) — treat as an unvalidated upper bound, not a result. "Both (combined)" mode (same pair's Asia+Monday trades sharing one concurrency budget) pushes Sharpe even higher (9.42), reinforcing rather than allaying that concern. No correlated-risk leave-one-out study run for this engine yet (unlike Level Atlas's) — page says so plainly, doesn't imply a validated exclusion list. See `LEGO_MODULES.md` |
...
| `market-sense.html` | Runs the pre-registered tests in `MARKET_SENSE_TESTS.md` (S1-S12, S17) by button click, with live log + generic results view; S13-S16 listed but disabled (harness code not currently in the file) | 🔬 Research | 2026-09-18 | Needs OANDA (+FRED/CFTC/this desk's own API for some studies) — Railway only, 500s in a sandboxed session |
...
| `impulse-range-lab.html` | Research-only: tests whether a significant 4H impulse candle works better as a reference RANGE (levels + ±0.25R/0.5R/0.75R/1.0R extensions that 3m/1m price tests for continuation/fade) than as a standalone directional signal, conditioned on Asia/Monday range confluence (`js/confluence-core.js`, reused not reimplemented) and VWAP context — deterministic CONTINUATION/REVERSION/FAILED_IMPULSE/EXTENSION/NO_CLEAR_EDGE outcome classification + MFE/MAE + an explicitly-unvalidated experimental confluence score, `js/impulseRangeEngine.js` | 🔬 Research | 2026-08-23 | Strictly causal (impulse qualification never sees future H4 bars); M1 fetch is new (`/api/ohlc-range?granularity=M1`) and untested live — needs a real check once deployed |
...
| `fx-vol-carry-backtest.html` | Tests implied-vs-realized vol spread (VRP) as a gate on the existing fade/follow exhaustion band vs always-fade/always-follow baselines, IS/OOS, 3 CSV exports (`/api/fx-vol-carry/run`+`/status`). CME CVOL for 6 FX pairs + Gold; CBOE VXN is NQ's only implied-vol source (CVOL has no index coverage); CBOE GVZ shown as an independent cross-check on Gold's CVOL read (correlation + overlay, not a second trading arm) | 🔬 Research | 2026-08-26 | Both CVOL (`js/data/cmeCvolEod.json`) and CBOE (`js/data/cboeVolIndices.json`) data are static snapshots, not live. First live run (2026-08-22) found the vrp arm at 0 trades on all 8 instruments — a real bug (CME/CBOE holiday-calendar gaps left the rolling z-score NaN on literally every day of the 11-year history), fixed 2026-08-26. Needs a re-run to confirm the fixed numbers |
