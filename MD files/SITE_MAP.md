...
| `level-atlas-vote-backtest.html` | Level Atlas vote-margin backtest visualized on real M1 candles — entry/exit markers, target/stop lines, per-trade decision/margin, filterable/sortable trade table | 🔬 Research | 2026-08-26 | Fixed-barrier P&L (not MFE/MAE), walk-forward positive at margin≥3-4 across 5 pairs — see `LEGO_MODULES.md` for the full result and caveats (correlated trades, no position-sizing policy) before treating as tradeable |
