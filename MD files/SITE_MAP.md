...
| `fx-vol-carry-backtest.html` | Tests implied-vs-realized vol spread (VRP) as a gate on the existing fade/follow exhaustion band vs always-fade/always-follow baselines, IS/OOS, 3 CSV exports (`/api/fx-vol-carry/run`+`/status`). CME CVOL for 6 FX pairs + Gold; CBOE VXN is NQ's only implied-vol source (CVOL has no index coverage); CBOE GVZ shown as an independent cross-check on Gold's CVOL read (correlation + overlay, not a second trading arm) | 🔬 Research | 2026-08-22 | Both CVOL (`js/data/cmeCvolEod.json`) and CBOE (`js/data/cboeVolIndices.json`) data are static snapshots, not live. Live OANDA fetch untestable in this sandbox (403) — needs a real run on Railway before its verdict badges mean anything |
